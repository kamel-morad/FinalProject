...
def _check_exist_cmpop(string_to_check):
    ...
    #Check if the given string contains a comparison operator
...
def _get_opposite_cmpop(cmpop):
    ...
    #Get the opposite comparison operator for the given operator
...
def _get_opposite_eh(eh):
    ...
    #Get the opposite comparison operator or return the original string if it doesn't contain a comparison operator
...
def translate_spec_item(spec_item, define=None):
    ...
    #Translate a specification item using the given define dictionary
...
def load_define(define_file_path):
    ...
    #Loads the macro definitions from the given file and returns them as a dictionary.
    #param define_file_path: The path to the file containing the macro definitions
    #return: A dictionary containing the macro definitions
...
def main_func(full_spec_file, define_file):
    ...
    #Main function that loads the spec file, translates it, and writes the output to a file
    ...
    with open(spec_file_location) as f:
        spec_list = yaml.full_load(f.read())
