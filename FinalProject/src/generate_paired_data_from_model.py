...
# The generate_paired_data_from_fixer function takes in a directory prefix, a prediction file 
# name, and an output directory. It collects successful predictions and appends the source 
# code (bad), fixed code (good), and an identifier to their respective lists. It then creates a new
# directory to store the new data, asserts that the number of good and bad examples are the 
# same, and writes the new data to the corresponding files. It loads the round 0 paired data, 
# shuffles the indices, and concatenates the indices for the round 0 data and the new data. 
# Finally, it writes the corresponding good, bad and id examples to the final output directory.

def generate_paired_data_from_fixer(pred_dir_prefix, pred_fname, out_dir):
    #Get new paired data
...
# The function generates new paired data from fixer predictions by collecting successful 
# predictions from the evaluation files of splits 0, 1, and 2, and appending the source code
# (bad), fixed code (good), and an identifier to their respective lists in a dictionary called 
# train_data. It then asserts that the number of good and bad examples are the same, creates 
# a new directory to store the new data, and writes the new data to the corresponding files. It 
# then loads the round 0 paired data, shuffles the indices, and concatenates the indices for the
# round 0 data and the new data. Finally, it writes the corresponding good, bad and id 
# examples to the final output directory. It is a backtranslation version and does not use critic.


def generate_paired_data_from_fixer_preds(pred_dir_prefix, pred_fname, out_dir):
    #Get new paired data
    train_data = {'good': [], 'bad': [], 'id': []}
    for split in {0,1,2}: #available for training
      print ('split', split)
...
parser.add_argument('--bugdetector', type=int, default=1)
...
if args.bugdetector:
    generate_paired_data_from_fixer(pred_dir_prefix, pred_fname, out_dir)
else:
    generate_paired_data_from_fixer_preds(pred_dir_prefix, pred_fname, out_dir)
