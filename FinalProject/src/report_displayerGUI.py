...
class ReportDisplayer:
    def __init__(self, report_file_path=None):
        root = tk.Tk()
        root.geometry('1220x670')
        root.title('Report Displayer')
        ...
        main_frame.grid(row=0, column=0, sticky='nswe',padx=5,pady=5)
        ...
        tk.Label(api_combobox_frame, text='Reason:').pack(side=tk.LEFT, padx=(80, 0))
        ...
        tk.Label(api_combobox_frame, text='Spec:').pack(side=tk.RIGHT,padx=0)

        # bug display frame
        bug_display_root = tk.Frame(main_frame, padx=5, pady=5,relief=tk.RAISED,borderwidth=1)
        ...
        bug_list_frame = tk.Frame(bug_display_root, highlightbackground='black', highlightcolor='black',
                                  highlightthickness=1, bd=0,relief=tk.RAISED,borderwidth=1)
        ...
        # bad usage display frame
        bad_display_frame = tk.Frame(bug_display_root, highlightbackground='black', highlightcolor='black',
                                        highlightthickness=1, bd=0,relief=tk.RAISED,borderwidth=1)
        bad_display_frame.pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(bad_display_frame, text='Bad Usage Display').pack(side=tk.TOP)
        bad_usage_container = tk.Frame(bad_display_frame)
        ...
        bad_usage_display_hsb = tk.Scrollbar(bad_display_frame, orient='horizontal',
        ...
        # good usage display frame
        good_usage_display_frame = tk.Frame(bug_display_root, highlightbackground='black', highlightcolor='black',
                                     highlightthickness=1, bd=0,relief=tk.RAISED,borderwidth=1)
        good_usage_display_frame.pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(good_usage_display_frame, text='Good Usage Display').pack(side=tk.TOP)
        good_usage_container = tk.Frame(good_usage_display_frame)
        ...
        good_usage_display_hsb = tk.Scrollbar(good_usage_display_frame, orient='horizontal',
        ...
        # Available fixes frame
        available_fix_frame = tk.Frame(bug_display_root, highlightbackground='black', highlightcolor='black',
                                  highlightthickness=1, bd=0,relief=tk.RAISED,borderwidth=1)
        available_fix_frame.pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(available_fix_frame, text='Available Fixes').pack(side=tk.TOP)
        fix_list_container = tk.Frame(available_fix_frame)
        fix_list_container.pack(side=tk.TOP, fill=tk.BOTH)
        self.ref_list = tk.Listbox(fix_list_container, width=20, height=30)
        ...
        fix_list_vert = tk.Scrollbar(fix_list_container, orient='vertical', command=self.ref_list.yview)
        fix_list_horz = tk.Scrollbar(available_fix_frame, orient='horizontal', command=self.ref_list.xview)
        self.ref_list.config(yscrollcommand=fix_list_vert.set, xscrollcommand=fix_list_horz.set)
        fix_list_vert.pack(side=tk.RIGHT, fill=tk.Y)
        fix_list_horz.pack(side=tk.BOTTOM, fill=tk.X)
    ...
    def _load_report(self, report_file_path):
        ...
        with open(report_file_path, "r") as f:
            ...
            file_data = file_data.replace("/mnt/c/FinalProject/src" , str(os.path.abspath('.')))
            full_report = yaml.safe_load(file_data)
            print(yaml.safe_dump(full_report))
        paths = []
